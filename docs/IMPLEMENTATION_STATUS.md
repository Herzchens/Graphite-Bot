...
| 5 | Account / Activity progression | Canonical Account XP curve/cap and fixed level Money settlement, one authoritative spendable Activity EXP pool with derived Minecraft-style level math, fixed-point Rebirth utility formulas, and idempotent Rebirth reset are implemented in the progression domain; Discord command wiring and live chat/gameplay/Quest source adapters remain pending |
...
- Progression events are append-only, progression mutations are operation-idempotent, and canonical state plus ledger/outbox/event provenance commits atomically.
