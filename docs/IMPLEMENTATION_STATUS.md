...
| 6 | Repair / Forge / Smelt / Enchant / +N / SoulBind | Smelting policy/preview foundation plus persistent per-job service identity and transaction-composable job-scoped stack reservation ownership implemented; Smelting runtime/settlement, production resource ItemDefinitions, live Confirm/commands, and remaining service systems are pending |
| 7 | Fishing | Pending |
...
Phase 6 has a read-only Smelting policy kernel, versioned ordinary-smelting recipe mappings, persistent service-job identity, and per-job stack reservation provenance, but `/smelt` and Confirm remain unregistered. Preview never consumes assets. The reservation primitive is caller-transaction-composable and is safe for already-existing versioned stack definitions; it does not itself commit the owning operation/outbox. Production Smelting remains blocked until resource catalog rows have authoritative stack ItemDefinitions/stack caps and runtime/output settlement plus the higher-level atomic Confirm flow exist. The retired aggregate `JOB_RESERVATION` Item Stack location is rejected at the database layer; job ownership lives in `service_job_stack_reservations`. The preview kernel also does not invent a mixed-fuel selection rule; it computes one explicitly selected canonical fuel kind at a time.
