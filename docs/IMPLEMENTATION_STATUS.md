...
| 6 | Repair / Forge / Smelt / Enchant / +N / SoulBind | Smelting policy/preview, persistent per-job reservation ownership, immutable tickless runtime snapshots, pure freeze-aware terminal consequence planning, and a transaction-composable pending-safe stack-delivery primitive implemented; authoritative Hard Freeze overlap tracking, production resource definitions/bridge, recipe/output identity, atomic terminal Smelting settlement, live Confirm/commands, and remaining service systems are pending |
| 7 | Fishing | Pending |
