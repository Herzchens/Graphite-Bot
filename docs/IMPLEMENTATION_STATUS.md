...
| 6 | Repair / Forge / Smelt / Enchant / +N / SoulBind | Smelting policy/preview foundation implemented with exact integer fuel heat, 20-second base unit time, partial-fuel preview, stop/cancel fuel accounting, and per-job AEXP remainder math; authoritative job reservation/settlement and the remaining service systems are pending |
| 7 | Fishing | Pending |
...
Phase 6 now has a read-only Smelting policy kernel and versioned ordinary-smelting recipe mappings, but `/smelt` and Confirm remain unregistered. Preview never consumes assets. Confirmation is intentionally blocked until resource catalog rows have authoritative stack ItemDefinitions and parallel service reservations are owned per job. The existing generic `JOB_RESERVATION` stack location is not treated as sufficient ownership for concurrent jobs because its current key has no job identifier. The preview kernel also does not invent a mixed-fuel selection rule; it computes one explicitly selected canonical fuel kind at a time.
