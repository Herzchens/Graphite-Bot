...
| 4 | Fixed NPC price/content registry | Frozen versioned resource lattice, NPC-buy/appraisal separation, Shop availability/stock classes, processed-metal formula regression, canonical alloy recipes, and v2 ordinary-smelting recipe mappings implemented; live Shop/NPC transactions remain pending their service slice |
...
| 6 | Repair / Forge / Smelt / Enchant / +N / SoulBind | Pure Repair economic preview/cancel-refund policy, Smelting policy/preview, persistent per-job reservation ownership, immutable tickless runtime snapshots, pure freeze-aware terminal consequence planning, and a transaction-composable pending-safe stack-delivery primitive implemented; live Repair lifecycle/time/modifiers, authoritative Hard Freeze overlap tracking, production resource definitions/bridge, recipe/output identity, atomic terminal Smelting settlement, live Confirm/commands, and remaining service systems are pending |
| 7 | Fishing | Pending |
...
- The legacy aggregate `item_stacks.location = 'JOB_RESERVATION'` path is rejected for both INSERT and UPDATE, preventing parallel jobs from collapsing ownership into the old `(player, definition, version, location)` key.
