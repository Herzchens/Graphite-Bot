...
- Account XP settlement is transaction-composable and mutation-keyed; exact replay returns the stored child receipt, cap handling and Account Level reward projection share the canonical progression arithmetic owner, and any synchronous level Money reward uses the existing balanced `LEVEL_REWARD` ledger authority.
- Account XP prelock acquires balance before progression/item state for cross-domain owners that may synchronously pay an Account Level reward; an owning operation with a pre-existing monetary ledger row fails closed under the current one-ledger-row-per-operation schema.
