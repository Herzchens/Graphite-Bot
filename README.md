...
- Typed embedded-enchant persistence for per-ItemInstance enchant identity plus resulting Level I–X, with immutable row identity, mutable level/removal state, equipment-parent validation, and no duplicated catalog/appraisal-class cache.
- Owner-scoped transaction-composable ordinary ItemInstance→`RecraftAppraisal` and ItemInstance→`EnhancedCanonicalAppraisal` resolution from the exact pinned definition version, locked Creation Roll/+N state, and locked embedded-enchant rows, with current-v1 ordinary tier/slot validation and fail-closed canonical enchant identity/level checks.
...
The equipment-appraisal foundation combines pure policy math with owner-scoped ordinary structural and enhanced resolvers. Ordinary equipment resolves `BaseEquipmentAppraisal = round100(TierAnchor × SlotFactor)` from the frozen nine-tier/seven-slot tables using checked integer/rational arithmetic, while an explicit ItemDefinition `base_appraisal` override takes exact precedence without additional rounding. Embedded enchants are valued from their canonical identity-derived acquisition/value class and resulting Level I–X using the frozen `60,000 × AcquisitionWeight × LevelMultiplier` table; the aggregate contribution is `round_half_up(0.70 × ΣBookAppraisal)`. +N power/appraisal contribution is represented exactly from `MainMult(N) = 1 + 0.03238N + 0.000952N²` and `UpgradeFactor(N) = 1 + 0.55 × (MainMult(N) - 1)` using checked rational intermediates. Creation Roll is accepted as a validated normalized exact rational `q∈[0,1]`, so the pure policy does not invent a storage precision. The composition kernel evaluates exact `BaseEquipmentAppraisal × RollFactor(q) × UpgradeFactor(N)`, applies one final structural round-half-up to produce `RecraftAppraisal`, then adds the already-integer `EmbeddedEnchantValue`; this is algebraically identical to the specification's final `round_half_up(structural + EmbeddedEnchantValue)` and therefore produces `EnhancedCanonicalAppraisal` without an unnecessary large-denominator multiplication. Fraction products reduce and cross-cancel before checked multiplication. Shadow Walker is represented by the mid-high class at its resulting level, while SoulBind is deliberately absent because it is not an enchant and contributes zero to its own appraisal. Starter Leather has no canonical TierAnchor in the active table, so it requires an explicit definition override rather than borrowing another material's anchor. ItemInstances persist Creation Roll as a canonical reduced exact rational plus mutable +N, and the item domain exposes an owner-scoped transaction-composable resolver for that raw structural state. Embedded enchant rows persist raw canonical identity plus resulting level separately from generic ItemInstance JSON; row identity is immutable while level decay and explicit removal remain representable. Services resolves authoritative ordinary `RecraftAppraisal` from locked structural state and the exact pinned immutable ItemDefinition tier/slot metadata, then can extend the same caller-owned transaction by locking embedded-enchant rows in deterministic key order, mapping exact persisted keys through the canonical catalog, enforcing the frozen per-enchant resulting-level ceilings, deriving appraisal classes, and resolving the ordinary ItemInstance's full `EnhancedCanonicalAppraisal`. Unknown keys and impossible persisted levels fail closed. Special ItemDefinition `base_appraisal` overrides remain outside this ordinary resolver, and the owning Enchant Apply/Combine/Remove lifecycle plus slot-capacity/conflict mutation enforcement are still not implemented. The full +N attempt-cost path also remains pending because the specification does not freeze a deterministic evaluation algorithm for `round10(20 × N^1.55)`.
