...
use crate::{
    OrdinaryEquipmentEnhancedResolverError, SlotOrbAttemptPreview, SlotOrbFamily, SlotOrbPolicyError,
    SlotOrbUnlock, lock_owned_ordinary_equipment_enhanced_appraisal, preview_slot_orb_attempt,
    slot_orb_policy,
};
...
impl std::fmt::Display for SlotOrbCapacityStateError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match *self {
            Self::PredecessorSlotsLocked {
                family,
                required_unlocked_slots,
                current_unlocked_slots,
            } => write!(
                formatter,
                "Slot Orb {:?} family requires {} predecessor slots unlocked; current capacity is {}",
                family, required_unlocked_slots, current_unlocked_slots
            ),
            Self::TargetSlotAlreadyUnlocked {
                family,
                target_slot_number,
                current_unlocked_slots,
            } => write!(
                formatter,
                "Slot Orb {:?} target slot #{} is already unlocked; current capacity is {}",
                family, target_slot_number, current_unlocked_slots
            ),
        }
    }
}

impl std::error::Error for SlotOrbCapacityStateError {}
