...
pub enum EquippedFishingRodCastSnapshotError {
    ...
    #[error("persisted Fishing Rod enchants {left:?} and {right:?} conflict at scope {scope:?}")]
...
pub async fn lock_equipped_fishing_rod_cast_snapshot(
    ...
    let mut embedded_enchants: Vec<EquippedFishingRodEnchantState> = Vec::with_capacity(rows.len());
    for row in rows {
        ...
        for existing in &embedded_enchants {
            if let Some(scope) = canonical_enchant_conflict_scope(existing.enchant, enchant) {
                return Err(
                    EquippedFishingRodCastSnapshotError::EmbeddedEnchantConflict {
        ...
        match placement.slot_family {
            EnchantSlotFamily::NormalClass => {
                normal_occupied = normal_occupied
                    .checked_add(1)
                    .ok_or(EquippedFishingRodCastSnapshotError::TooManyEmbeddedEnchantRows)?;
            }
