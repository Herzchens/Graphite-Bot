use graphite_services::{
    CanonicalEnchant, EnchantApplyAction, OrdinaryEnchantApplyStateWriterError,
...
#[tokio::test]
async fn state_writer_fails_closed_before_loadout_scoped_armor_mutation() {
    let Some(store) = test_store().await else {
        return;
    };
    let nonce = Uuid::now_v7();
    let owner_id = seed_player(&store, positive_snowflake(nonce)).await;
    let definition_key = format!("test.enchant-apply-writer.armor.{nonce}");
    seed_definition(
        &store,
        &definition_key,
        "ARMOR",
    ...
    .await;
    let item_id = seed_item(&store, owner_id, &definition_key, &nonce, "loadout").await;
    seed_structural_state(&store, item_id).await;
    ...
    assert!(matches!(
        ...
        .await,
        Err(OrdinaryEnchantApplyStateWriterError::EquippedArmorLoadoutValidationRequired)
    ));
